...
test-server:
	...
	curl -X POST -H "Content-Type:application/octet-stream" -d "Pizza is tasty" localhost:1212
	curl -X POST -H "Content-Type:application/octet-stream" -d "Cold weather is the worst" localhost:1212
